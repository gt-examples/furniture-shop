...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("fs-cart");
    if (stored) {
  ...
  useEffect(() => {
    if (loaded) localStorage.setItem("fs-cart", JSON.stringify(items));
  }, [items, loaded]);
  ...
  const updateQuantity = useCallback((productId: string, qty: number) => {
    ...
    else setItems((prev) => prev.map((i) => i.productId === productId ? { ...i, quantity: qty } : i));
  }, []);
